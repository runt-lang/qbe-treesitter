...
module.exports = grammar({
  ...
  rules: {
    ...
    data_item: $ => choice(
      seq($.extended_type, repeat1($._data_value)),
      seq('z', $.integer),
    ),
    ...
    linkage: $ => choice(
      'export',
      'thread',
      seq('section', $.string, optional($.string)),
    ),
    ...
    binary_op: $ => seq(
      field('op', $.binary_opcode),
      $._value,
      ',',
      $._value,
    ),

    binary_opcode: $ => choice(
    ...
    compare_op: $ => seq(
      field('op', $.compare_opcode),
      $._value,
      ',',
      $._value,
    ),

    compare_opcode: $ => choice(
      // Integer comparisons (w/l)
      'ceqw', 'cnew', 'csltw', 'cslew', 'csgtw', 'csgew',
      'cultw', 'culew', 'cugtw', 'cugew',
      'ceql', 'cnel', 'csltl', 'cslel', 'csgtl', 'csgel',
      'cultl', 'culel', 'cugtl', 'cugel',
      // Floating comparisons (s/d)
      'ceqs', 'cnes', 'clts', 'cles', 'cgts', 'cges', 'cos', 'cuos',
      'ceqd', 'cned', 'cltd', 'cled', 'cgtd', 'cged', 'cod', 'cuod',
    ),

    convert_op: $ => seq(
      field('op', $.convert_opcode),
      $._value,
    ),

    convert_opcode: $ => choice(
    ...
    load_op: $ => seq(
      field('op', $.load_opcode),
      $._value,
    ),

    load_opcode: $ => choice(
      'loadw', 'loadl', 'loads', 'loadd',
      'loadsw', 'loaduw', 'loadsb', 'loadub', 'loadsh', 'loaduh',
    ),

    store_inst: $ => seq(
      field('op', $.store_opcode),
      $._value,
      ',',
      $._value,
    ),
    ...
    store_opcode: $ => choice(
      'storew', 'storel', 'stores', 'stored', 'storeb', 'storeh',
    ),

    alloc_op: $ => seq(
      field('op', $.alloc_opcode),
      $._value,
    ),
    ...
    call_arg: $ => choice(
      seq(optional('env'), $.abi_type, $._value),
      '...',
    ),
